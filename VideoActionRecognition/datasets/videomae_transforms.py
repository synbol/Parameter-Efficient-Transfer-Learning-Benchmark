...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_tuple):
        ...
        im_size = img_group[0].size  # (454, 256)
...
class RandomCrop(object):
    def __init__(self, min_size=256, max_size=320, crop_size=224, inverse_uniform_sampling=True):
    ...
    def __call__(self, img_tuple):
        ...
        width, height = images[0].size  # first w, then h
        ...
        resized_images = [img.resize((new_width, new_height), Image.BILINEAR) for img in images]
        ...
        cropped_images = [img.crop((x_offset, y_offset, x_offset + size, y_offset + size)) for img in images]

        return cropped_images, label