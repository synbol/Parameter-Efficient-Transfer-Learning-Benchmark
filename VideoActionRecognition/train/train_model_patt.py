# --------------------------------------------------------
# References:
# A Unified Framework for Parameter-Efficient Transfer Learning：https://github.com/jxhe/unify-parameter-efficient-tuning
...
import models
from models.video_swin_transformer_patt import *
...
def get_args_parser():
    ...
    parser.add_argument('--att_prefix_mode', default='prompt_qk',
                        choices=['patt_kv','patt_qv','patt_qk','patt_qkv',
                        'prefix_kv', 'prefix_qk'], type=str, help='dataset')
    parser.add_argument('--att_prefix_scale', default=0.2, type=float)
    parser.add_argument('--att_prefix', default=False, action='store_true', help='whether activate AdaptFormer')
    parser.add_argument('--att_preseqlen', default=20, type=int, help='bottleneck middle dimension')
    parser.add_argument('--att_mid_dim', default=64, type=int, help='bottleneck middle dimension')
    parser.add_argument('--tuned_backbone_layer_fc', '--tbl_fc',  default='False', type=str, help='configure the tuned layer')
...
def main(args):
    ...
    tuning_config = EasyDict(
        ...
        # Prefix
        att_prefix_mode=args.att_prefix_mode,
        att_prefix_scale=args.att_prefix_scale,
        att_prefix=args.att_prefix,
        att_preseqlen=args.att_preseqlen,
        att_mid_dim=args.att_mid_dim,
    )
    ...
    if args.model.startswith('swin_'):
        ...
        model.fc = nn.Sequential(nn.Conv3d(1024, args.nb_classes, kernel_size=1, stride=1, bias=True),)
    ...
    if args.finetune and not args.eval and not args.model.startswith('s3d') and not args.model.startswith('swin'):
    ...
    if args.tuned_backbone_layer_fc.lower() == 'false':
        for name2, params in model.fc.named_parameters():
    ...
    n_parameters_freeze = sum(p.numel() for p in model.parameters() if not p.requires_grad)

    print('number of params tune   (M): %.5f' % (n_parameters / 1.e6))
    print('number of params freeze (M): %.5f' % (n_parameters_freeze / 1.e6))
    ...
    if args.eval:
        ...
        print(f'Max accuracy: {test_stats["acc1"]:.2f}%')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"Accuracy of the network on the {len(data_loader_test)} test images: {test_stats['acc1']:.2f}%")
    ...
    test_stats = evaluate(data_loader_val, model_best, device)
    preds_file = os.path.join(args.output_dir, str(global_rank) + '.txt')
    test_stats = final_test(data_loader_test, model_best, device, preds_file)
    torch.distributed.barrier()
    if global_rank == 0:
        print("Start merging results...")
        final_top1, final_top5 = merge(args.output_dir, num_tasks, is_hmdb=args.data_set == 'HMDB51')
