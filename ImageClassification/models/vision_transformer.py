...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., r=4, lora_alpha=4, tuning_mode=None, config=None):
        super().__init__()
        ...
        if tuning_mode == 'lora':
            ...
            self.w_As = []
            self.w_Bs = []
            w_dim = self.qkv.in_features
            self.dim =self.qkv.in_features
            self.lora_a_q = nn.Linear(w_dim, r, bias=False)
            self.lora_a_v = nn.Linear(w_dim, r, bias=False)
            self.lora_b_q = nn.Linear(r, w_dim, bias=False)
            self.lora_b_v = nn.Linear(r, w_dim, bias=False)
            ...
            #self.dim = self.qkv.in_features
            #self.q_proj = lora.Linear(dim, dim, r=4).to("cuda")
            #self.k_proj = nn.Linear(dim, dim).to("cuda")
            #self.v_proj = lora.Linear(dim, dim, r=4).to("cuda")
            #self.qkv_proj = lora.MergedLinear(dim, 3*dim, r=4, enable_lora=[True, False, True]).to("cuda:0")
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'ssf':
            ...
            new_q = self.lora_b_q(self.lora_a_q(x)) * self.lora_scale
            new_v = self.lora_b_v(self.lora_a_v(x)) * self.lora_scale
            qkv[:, :, : self.dim] += new_q
            qkv[:, :, -self.dim :] += new_v
...
class Block(nn.Module):
    ...
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, tuning_mode=None, config=None):
        super().__init__()
        self.dim = dim
        self.config = config
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, tuning_mode=tuning_mode, config=config)
        ...
        if tuning_mode == 'ssf':     
            ...
            self.ssf_scale_2, self.ssf_shift_2 = init_ssf_scale_shift(dim)
        elif tuning_mode == 'adapter':
            self.adaptmlp = Adapter(self.config, dropout=0.1, init_option="lora", adapter_scalar="1.0", adapter_layernorm_option="none", bottleneck=config.adapt_bottleneck, d_model=768)
            # pass
    ...
    def forward(self, x):
        if self.tuning_mode == 'ssf':
            ...
            # TODO: implement the detail of AdapterFormer
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, tuning_mode=None, tuning_config=None): 
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, tuning_mode=tuning_mode_list[i], config=tuning_config)
        ...
        if self.tuning_mode == 'vpt':
            assert tuning_config.vpt_num > 0
            self.embeddings = nn.ParameterList(
                [nn.Parameter(torch.empty(1, tuning_config.vpt_num, self.embed_dim)) for _ in range(depth)]
            )
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.tuning_mode == 'vpt':
                x = torch.cat((
                    x[:, :1, :],
                    self.embeddings[0].expand(B, -1, -1),
                    x[:, 1:, :]
                ), dim = 1)
                '''
