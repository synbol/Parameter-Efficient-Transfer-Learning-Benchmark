...
def create_optimizer_v2(
    ...
    if isinstance(model_or_params, nn.Module):
        ...
        if tuning_mode:
            for name, param in model_or_params.named_parameters():
                if tuning_mode == 'linear_probe':
                    ...
                    if "head." not in name and "lora_" not in name:
                        param.requires_grad = False
                elif tuning_mode == 'adapter':
                    if "head." not in name and "adapt" not in name:
                        param.requires_grad = False
                elif tuning_mode == 'vpt':
                    if "head." not in name and 'embeddings.0' not in name:
                        param.requires_grad = False
        ...
        elif layer_decay is not None: # layer_decay: NONE
